...
<div>
    <el-form inline label-width="180px">
        ...
        <el-form-item label="装置类别" style="text-align:left">
            <el-select v-model="system_type" filterable allow-create clearable>
                <el-option label="炼油装置（燃料油）" value="炼油装置（燃料油）">
                </el-option>
        ...
        <el-form-item label="工艺类型" style="text-align:left">
            <el-select v-model="process_type" filterable allow-create clearable>
        ...
        <el-form-item label="原料密度（kg/m³）">
            <el-form-item>
                <el-input placeholder="密度下界" v-model="density_l" clearable>
                </el-input>
            </el-form-item>
            -
            <el-form-item>
                <el-input placeholder="密度上界" v-model="density_u" clearable>
                </el-input>
            </el-form-item>
        </el-form-item>
        <br />
    ...
    <el-table :data="tableData" border style="width: 100%" max-height="600">
        ...
        <el-table-column fixed="right" label="操作" width="100">
            <template slot-scope="scope">
                <router-link :to="{name: 'search_bar', query: {id: scope.row.id}}" target='_blank'>
                    <el-button  type="text" size="medium">跳转</el-button>
                </router-link> 
                <el-button @click="generateGraph(scope.row.id)" type="text" size="medium">图谱</el-button>
            </template>
        </el-table-column>
    </el-table>
    <router-view>
...
export default {
    ...
    methods: {
        generateGraph(){
            this.$router.push("/build_graph");
        },
        getSystemById(systemId) {
            console.log('get system by id: ' + systemId)
            var systemInfo = null
            this.$http
                .post(
            ...
            const serachPage  = this.$router.resolve({
                path: '/AVDU/search/bar',
                name: "search_bar",
                query:{
                    search_systemInfo: this.$store.search_systemInfo,
                    other: 1
                }
            });
            window.open(serachPage.href, '_blank')
        },
